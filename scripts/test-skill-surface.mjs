...
assert.match(skill, /call polar_connection_status/);
...
const result = await run(["call", "polar_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "polar_connection_status", files: pkg.files }, null, 2));
