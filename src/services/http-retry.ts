 ...
 * - Logs every retry to stderr as `[polar-mcp] retry N/3 after Xms (status=Y or error=Z)`.
 * - Honors `POLAR_NO_RETRY=true` env var to disable (used in tests).
 */
export const RETRYABLE_STATUSES = new Set([408, 429, 500, 502, 503, 504]);
export const MAX_ATTEMPTS = 3;
const BACKOFF_MS = [500, 1000, 2000];
const JITTER_FACTOR = 0.2;
const LOG_PREFIX = "[polar-mcp]";
...
function isNoRetryEnv(): boolean {
  const value = process.env.POLAR_NO_RETRY;
